...
#pragma warning restore 0618
        ...
        public InputDevice GetHMD()
        {
            ...
            InputDevices.GetDevicesWithCharacteristics(dc1, devices);
        ...
        {
            devices.Clear(); // Optional, only needed if devices was reused elsewhere
            leftHandedControllers.Clear();
            ...
            return leftHandedControllers.Count > 0 ? leftHandedControllers[0] : default;
        }


        public InputDevice GetRightController() {
            InputDevices.GetDevices(devices);
            rightHandedControllers.Clear();    
            ...
            return rightHandedControllers.Count > 0 ? rightHandedControllers[0] : default;
        }
        ...
        public virtual string GetControllerName()
        {
            ...
            var dc = InputDeviceCharacteristics.HeldInHand | InputDeviceCharacteristics.Right | InputDeviceCharacteristics.Controller;
            InputDevices.GetDevicesWithCharacteristics(dc, rightHandedControllers);
            InputDevice primaryRightController = rightHandedControllers.Count > 0 ? rightHandedControllers[0] : default;

            // Return name of the found controller
            if (primaryRightController.isValid && !string.IsNullOrEmpty(primaryRightController.name))
            {
            ...
            InputDevices.GetDevicesWithCharacteristics(dc, leftHandedControllers);
            InputDevice primaryLeftController = leftHandedControllers.Count > 0 ? leftHandedControllers[0] : default;

            if (primaryLeftController.isValid && !string.IsNullOrEmpty(primaryLeftController.name))
            {
...
#pragma warning restore
        ...
        public void GetBoundaryPoints()
        {
            ...
            if (subSystemsCount > 0)
            {
                for (int x = 0; x < subSystemsCount; x++)
                {
                    if (subsystems[x].TryGetBoundaryPoints(boundaryPoints))
                    {
                        setTrackingOrigin = true;
                        Debug.Log($"[Boundary] Successfully got {boundaryPoints.Count} points");
                    }
                    else
                    {
                        Debug.LogWarning("[Boundary] Failed to get boundary points");
                    }
                }
            }
            else
