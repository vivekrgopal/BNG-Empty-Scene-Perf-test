    ...
    public class Grabbable : MonoBehaviour {
        ...
        public virtual void FixedUpdate() {
            ...
            if (BeingHeld) {
                ...
                else if (GrabPhysics == GrabPhysics.Custom)
                    UpdateCustomPhysics();
                else if (GrabPhysics == GrabPhysics.Racket)
                    UpdateRacketPhysics();
            }
        }        
        ...
        public virtual void UpdateCustomPhysics()
        {
            ...
            setPositionSpring(10000f, 1f);
            setSlerpDrive(2000f, 10f);

            //Grabber grabber = GetPrimaryGrabber();
            //if (!grabber.GetGrabberAveragedVelocity().IsNan())
            //    rigid.velocity = grabber.GetGrabberAveragedVelocity();
            

            //if (!grabber.GetGrabberAveragedAngularVelocity().IsNan() && Mathf.Abs(grabber.GetGrabberAveragedAngularVelocity().x) != Mathf.Infinity)
            //{
            //    rigid.angularVelocity = grabber.GetGrabberAveragedAngularVelocity(); //Vector3.Lerp(rigid.angularVelocity, grabber.GetGrabberAveragedAngularVelocity(), Time.deltaTime * CustomRotationSpeed);
        ...
        public virtual void UpdateRacketPhysics()
        {
            //rigid.MovePosition(grabTransform.position);
            //rigid.MoveRotation(grabTransform.rotation);
            rigid.transform.position = grabTransform.position;
            rigid.transform.rotation = grabTransform.rotation;

            //Grabber grabber = GetPrimaryGrabber();
            //if(!grabber.GetGrabberAveragedVelocity().IsNan())
            //    rigid.velocity = grabber.GetGrabberAveragedVelocity();
            //if(!grabber.GetGrabberAveragedVelocity().IsNan())
            //    rigid.angularVelocity = grabber.GetGrabberAveragedAngularVelocity();
        }
