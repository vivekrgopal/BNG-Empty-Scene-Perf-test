...
namespace BNG {
    ...
    public class SmoothLocomotion : MonoBehaviour {
        ...
        public float Deacceleartion = 20f;
        ...
        public virtual void Update() {
            ...
            //bool isInFitness = GameManager.Instance != null && GameManager.Instance.CurrentGameMode == GameMode.TRAINING_AND_FITNESS && GameManager.Instance.TrainingAndFitnessManager.CurrentWorkoutType == WorkoutType.FITNESS;
            //bool isInMR = PlayerManager.Instance != null && (PlayerManager.Instance.Player as VRTennisPlayer).IsMRMode;

            //if (UpdateMovement && !isInMR && !isInFitness) 
            //{
            //    MoveCharacter();
            //}
        }
        ...
        public virtual void UpdateInputs() {
            ...
            currentXSpeed = UpdateMoveSpeed(movementX, currentXSpeed);
            currentZSpeed = UpdateMoveSpeed(movementZ, currentZSpeed);
            ...
            movementZ *= currentZSpeed;

            // Attach any additional speed
            //if(CheckSprint()) {
            //    movementX *= StrafeScurrentSpeedprintSpeed;
        ...
        private float UpdateMoveSpeed(float inputDirection, float currentSpeed)
        {
            if (inputDirection > 0 || inputDirection < 0)
            {
                if (CheckSprint())
                {
                    if (currentSpeed < SprintSpeed)
                    {
                        Mathf.Clamp(currentSpeed += Time.deltaTime * Acceleration, 0f, SprintSpeed);
                    }
                    else
                ...
                else
                {
                    if (currentSpeed >= SprintSpeed)
                    {
                        Mathf.Clamp(currentSpeed -= Time.deltaTime * Deacceleartion, 0f, SprintSpeed);
                    }
                    else if (currentSpeed < MovementSpeed)
                    {
                        Mathf.Clamp(currentSpeed += Time.deltaTime * Acceleration, 0f, MovementSpeed);
                    }
                    else
